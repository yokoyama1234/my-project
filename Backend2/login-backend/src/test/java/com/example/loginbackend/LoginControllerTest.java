...
/**
 * {@link LoginController} の単体テストクラス（完全版）。
 *
 * <p>
 * SessionService を Mock 化し、コントローラ層の動作を検証する。
 * JSON のユーザー名フィールドは "name" に統一。
 * </p>
 */
...
class LoginControllerTest {
    ...
    class LoginApiTests {
        ...
        void testLoginSuccess() throws Exception {
            ...
            when(loginService.login(FIXED_USER, FIXED_PASS)).thenReturn(user);
            ...
            verify(sessionService).setUser(any(), eq(user));
        }
    ...
    class MeApiTests {
        ...
        void testMeWhenLoggedIn() throws Exception {
            ...
            when(messageSource.getMessage("login.already_logged_in", null, LOCALE))
                    .thenReturn("ログイン中");
